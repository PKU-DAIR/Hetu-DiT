...
from diffusers.models.attention import FeedForward, GELU, GEGLU, LinearActivation
...
import torch
from hetu_dit.model_executor.diffusion_executor.layers.base_layer import (
    hetuDiTDiffusionLayerBaseWrapper,
)
from hetu_dit.model_executor.utils.register_warpper import hetuDiTLayerWrappersRegister


@hetuDiTLayerWrappersRegister.register(FeedForward)
class hetuDiTFeedForwardWrapper(hetuDiTDiffusionLayerBaseWrapper):
    def __init__(self, feedforward: FeedForward):
        super(hetuDiTFeedForwardWrapper, self).__init__(module=feedforward)
        ...
        elif isinstance(self.module.net[0], LinearActivation):
            self.module.net[0].proj.weight.data = self.module.net[
                0
            ].proj.weight.data.chunk(tp_degree, dim=0)[tp_rank]
            if self.module.net[0].proj.bias is not None:
