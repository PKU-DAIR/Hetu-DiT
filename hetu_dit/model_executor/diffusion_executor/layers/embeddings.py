...
@hetuDiTLayerWrappersRegister.register(PatchEmbed)
class hetuDiTPatchEmbedWrapper(hetuDiTDiffusionLayerBaseWrapper):
    def __init__(
        self,
    ...
    def forward(self, latent):
        ...
        if get_runtime_state().patch_mode:
            ...
            pos_embed_list = [
                pos_embed[
                    :,
                    get_runtime_state().pp_patches_token_start_end_idx_global[i][
                        0
                    ] : get_runtime_state().pp_patches_token_start_end_idx_global[i][1],
                    :,
                ]
...
@hetuDiTLayerWrappersRegister.register(CogVideoXPatchEmbed)
class hetuDiTCogVideoXPatchEmbedWrapper(hetuDiTDiffusionLayerBaseWrapper):
    def __init__(
        self,
    ...
    def forward(self, text_embeds: torch.Tensor, image_embeds: torch.Tensor):
        ...
        if self.use_positional_embeddings or self.use_learned_positional_embeddings:
            if self.use_learned_positional_embeddings and (
                self.sample_width != width or self.sample_height != sum_height
            ):
                raise ValueError(
            ...
            if (
                ...
                pos_embedding = self._get_positional_embeddings(
                    sum_height, width, pre_time_compression_frames
                )
            ...
            post_time_compression_frames = (
                pre_time_compression_frames - 1
            ) // self.temporal_compression_ratio + 1

            pos_embed_list = [
                pos_embedding[
                    :,
                    post_patch_height * post_patch_width * i
                    + get_runtime_state().pp_patches_token_start_end_idx_global[0][
                        0
                    ] : post_patch_height * post_patch_width * i
