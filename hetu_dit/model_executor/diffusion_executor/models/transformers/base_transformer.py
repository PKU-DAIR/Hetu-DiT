...
from hetu_dit.model_executor.diffusion_executor.models.base_model import (
...
class hetuDiTDiffusionTransformerBaseWrapper(
    hetuDiTDiffusionModelBaseWrapper, metaclass=ABCMeta
):
            ...
            if (
                ...
                if blocks_end <= stage_block_end_idx:
                    ...
                    setattr(
                        ...
                        name,
                        blocks_list[name][
                            stage_block_start_idx - blocks_start : stage_block_end_idx
                            - blocks_end
                        ],
                    )
                    self.stage_info.after_flags[name] = False
    ...
    def _set_transformer_blocks_metadata(
        self,
        cpu_transformer: nn.Module,
        blocks_name: List[str] = [],
    ):
        """
        Reuse of _split_transformer_blocks,
        """
        for block_name in blocks_name:
            if not hasattr(cpu_transformer, block_name):
                raise AttributeError(
                    f"'{cpu_transformer.__class__.__name__}' object has no attribute "
                    f"'{block_name}'."
                )
        ...
        pp_world_size = get_pipeline_parallel_world_size()
        blocks_list = {
            block_name: getattr(cpu_transformer, block_name)
            for block_name in blocks_name
        }
        num_blocks_list = [len(blocks) for blocks in blocks_list.values()]
        self.blocks_idx = {
