...
# from hetu_dit.model_executor.utils.register_warpper import hetu_ditPipelineWrapperRegister
...
@hetuDiTPipelineWrapperRegister.register(CogVideoXPipeline)
class hetuDiTCogVideoXPipeline(hetuDiTPipelineBaseWrapper):
    def __init__(
        self,
        ...
        T5encoder_model = getattr(pipeline, "text_encoder", None)
        if get_runtime_state().runtime_config.use_parallel_text_encoder:
            if T5encoder_model is not None:
                pipeline.text_encoder = self._convert_textencoder_backbone(
                    T5encoder_model
                )
        pipeline.vae.enable_tiling()

    def _convert_textencoder_backbone(self, textencoder: nn.Module):
        logger.info("Textencoder backbone found, paralleling textencoder...")
        wrapper = hetuDiTTextEncoderModelWrappersRegister.get_wrapper(textencoder)
        textencoder = wrapper(textencoder)
    ...
    def __call__(
        ...
        prompt_embeds, negative_prompt_embeds = encoder(
            self,
            prompt,
            negative_prompt,
        ...
        logger.info(
            f"prompt_embeds: {prompt_embeds.shape}, negative_prompt_embeds: {negative_prompt_embeds.shape}"
        )
        ...
        # 9. Prepare CogVideoX scheduler
        self.scheduler.set_timesteps(num_inference_steps, device=device)
        p_t = self.transformer.config.patch_size_t or 1
        # p_t = getattr(self.transformer.config, "patch_size_t", 1)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            latents, prompt_embeds, image_rotary_emb = self.denoise_loop(
                latents,
                prompt_embeds,
                image_rotary_emb,
                timesteps,
        ...
        def vae(self, *args, **kwargs):
            return self.decode_latents(*args, **kwargs)

        if is_dp_last_group():
            if not output_type == "latent":
                # Discard any padding frames that were added for CogVideoX 1.5
                latents = latents[:, additional_frames:]
                video = vae(self, latents)
