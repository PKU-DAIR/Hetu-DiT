...
from hetu_dit.config import EngineConfig, InputConfig
...
@hetuDiTPipelineWrapperRegister.register(FluxPipeline)
class hetuDiTFluxPipeline(hetuDiTPipelineBaseWrapper):
    def __init__(
        self,
        ...
        if get_runtime_state().runtime_config.use_parallel_text_encoder:
            T5encoder_model = getattr(pipeline, "text_encoder_2", None)
            if T5encoder_model is not None:
                pipeline.text_encoder_2 = self._convert_textencoder_backbone(
                    T5encoder_model
                )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        logger.debug(
            f"prompt_embeds.shape: {prompt_embeds.shape}, dtype: {prompt_embeds.dtype}, pooled_prompt_embeds.shape: {pooled_prompt_embeds.shape}, dtype: {pooled_prompt_embeds.dtype}, text_ids.shape: {text_ids.shape}, dtype: {text_ids.dtype}, {text_ids}"
        )
        ...
        logger.debug(
            f"num_pipeline_warmup_steps: {num_pipeline_warmup_steps}, num_warmup_steps: {num_warmup_steps}, num_inference_steps: {num_inference_steps}"
        )
        # 6. Denoising loop
    ...
    @hetuDiTPipelineBaseWrapper.profile_decorator("diffusion_async")
    def _async_pipeline(
        self,
        latents: torch.Tensor,
        prompt_embeds: torch.Tensor,
