...
@hetuDiTPipelineWrapperRegister.register(StableDiffusion3Pipeline)
class hetuDiTStableDiffusion3Pipeline(hetuDiTPipelineBaseWrapper):
    def __init__(
        self,
        ...
        if get_runtime_state().runtime_config.use_parallel_text_encoder:
            T5encoder_model = getattr(pipeline, "text_encoder_3", None)
            if T5encoder_model is not None:
                pipeline.text_encoder_3 = self._convert_textencoder_backbone(
                    T5encoder_model
                )
    ...
    def _sync_pipeline(
        ...
        logger.debug(
            f"in _sync_pipeline, before _init_sync_pipeline, latents.shape = {latents.shape}"
        )
        latents = self._init_sync_pipeline(latents)
        logger.debug(
            f"in _sync_pipeline, after _init_sync_pipeline, latents.shape = {latents.shape}"
        )
    ...
    def _async_pipeline(
        ...
        num_pipeline_warmup_steps = get_runtime_state().runtime_config.warmup_steps
        logger.debug(
            f"in _async_pipeline, before _init_ssync_pipeline, latents.shape = {latents.shape}"
        )
