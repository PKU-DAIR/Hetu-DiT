...
from hetu_dit.model_executor.vae_executor.models.layers.conv2d import PatchConv2d
from hetu_dit.model_executor.vae_executor.models.layers.normalization import (
    PatchGroupNorm,
)
from hetu_dit.model_executor.vae_executor.modules.patch_utils import (
    Patchify,
    DePatchify,
)
...
class PatchDecoder(nn.Module):
    ...
    def __init__(
        ...
        assert norm_type == "group", (
            "Only group normalization is supported in PatchDecoder. Please use Decoder instead."
        )
        super().__init__()
        for up_block in up_block_types:
            assert up_block in ["UpDecoderBlock2D"], (
                "Only UpDecoderBlock2D is supported in PatchDecoder. Please use Decoder instead."
            )
        self.layers_per_block = layers_per_block
        ...
        if norm_type == "spatial":
            ...
            self.conv_norm_out = PatchGroupNorm(
                num_channels=block_out_channels[0], num_groups=norm_num_groups, eps=1e-6
            )
        self.conv_act = nn.SiLU()
        self.conv_out = PatchConv2d(
            block_out_channels[0],
            out_channels,
            3,
            padding=1,
    ...
    def forward(
        ...
        if self.training and self.gradient_checkpointing:
            ...
            # middle
            logger.debug("before self.mid_block")
            sample = self.mid_block(sample, latent_embeds)
            sample = sample.to(upscale_dtype)
            logger.debug("after self.mid_block")
            # up
            sample = self.patch(sample)
            for up_block in self.up_blocks:
