...
class PatchAdaGroupNorm(nn.Module):
    ...
    def forward(self, x: torch.Tensor, emb: torch.Tensor) -> torch.Tensor:
        ...
        dist.all_gather(
            height_list,
            torch.tensor([x.shape[2]], dtype=torch.int64),
...
class PatchGroupNorm(nn.GroupNorm):
    ...
    def forward(self, x: Tensor) -> Tensor:
        # get height
        logger.debug(
            f"before patch group norm, get_world_group_rank: {get_world_group_rank()}, get_world_group_world_size: {get_world_group_world_size()}"
        )
        ...
        group_var_sum = torch.empty(
            (x.shape[0], self.num_groups), dtype=torch.float32, device=x.device
        )
        ...
        logger.debug(
            f"after patch group norm, get_world_group_rank: {get_world_group_rank()}, get_world_group_world_size: {get_world_group_world_size()}"
        )
        return x
