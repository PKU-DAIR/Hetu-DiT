...
class PatchConv2d(nn.Conv2d):
    ...
    def _get_world_size_and_rank(self):
        ...
        if dist.is_available() and dist.is_initialized():
            rank = get_world_group_rank()
            world_size = get_world_group_world_size()
    ...
    def _calc_bottom_halo_width(
        self, rank, height_index, kernel_size, padding=0, stride=1
    ):
        assert rank >= 0, "rank should not be smaller than 0"
        assert rank < len(height_index) - 1, (
            "rank should be smaller than the length of height_index - 1"
        )
        assert padding >= 0, "padding should not smaller than 0"
        assert stride > 0, "stride should be larger than 0"

        if rank == get_world_group_world_size() - 1:
            return 0
        nstep_before_bottom = (
            height_index[rank + 1] + padding - (kernel_size - 1) // 2 + stride - 1
        ) // stride
    ...
    def _calc_halo_width_in_h_dim(
        ...
        halo_width = [
            ...
            self._calc_bottom_halo_width(
                rank, height_index, kernel_size, padding, stride
            ),
        ]
        if rank == 0:
            halo_width[0] = 0
        elif rank == get_world_group_world_size() - 1:
            halo_width[1] = 0
    ...
    def _conv_forward(self, input: Tensor, weight: Tensor, bias: Optional[Tensor]):
        logger.debug(
            f"before PatchConv2d _conv_fwd: get_world_group_world_size(): {get_world_group_world_size()}, get_world_group_rank(): {get_world_group_rank()}"
        )
        ...
        if world_size == 1:
            ...
            patch_height_list = [
                torch.zeros(1, dtype=torch.int64, device=f"cuda:{0}")
                for _ in range(get_world_group_world_size())
            ]
            dist.all_gather(
                patch_height_list,
                torch.tensor([h], dtype=torch.int64, device=f"cuda:{0}"),
            ...
            assert halo_width[0] <= h and halo_width[1] <= h, (
                "halo width is larger than the height of input tensor"
            )
            ...
            if next_top_halo_width > 0:
                ...
                to_next = dist.isend(
                    bottom_halo_send,
                    get_world_group().ranks[(rank + 1) % get_world_group_world_size()],
                )

            if halo_width[0] > 0:
                # recv from prev
                assert (
                    patch_height_index[rank] - halo_width[0]
                    >= patch_height_index[rank - 1]
                ), (
                    "width of top halo region is larger than the height of input tensor of last rank"
                )
                top_halo_recv = torch.empty(
                    [bs, channels, halo_width[0], w],
                    dtype=input.dtype,
                    device=f"cuda:{0}",
                )
                dist.recv(
            ...
            padding = self._adjust_padding_for_patch(
                self._reversed_padding_repeated_twice, rank=rank, world_size=world_size
            )
            bs, channels, h, w = input.shape
            if self.block_size == 0 or (h <= self.block_size and w <= self.block_size):
                if self.padding_mode != "zeros":
                    conv_res = F.conv2d(
                        F.pad(input, padding, mode=self.padding_mode),
                        weight,
                        bias,
                        self.stride,
                        _pair(0),
                    ...
                    if (
                        self.stride[0] == 1
                        and self.padding[0] == 1
                        and self.kernel_size[0] == 3
                    ):
                        conv_res = F.conv2d(
                            input,
                            weight,
                        ...
                        conv_res = F.conv2d(
                            F.pad(input, padding, "constant", 0.0),
                            weight,
                            bias,
                            self.stride,
                            _pair(0),
                ...
                logger.debug(
                    f"after PatchConv2d _conv_fwd: get_world_group_world_size(): {get_world_group_world_size()}, get_world_group_rank(): {get_world_group_rank()}"
                )
                return conv_res
