...
# Adapted from
# https://github.com/xdit-project/xDiT/blob/main/xfuser/core/distributed/group_coordinator.py
...
def _is_cpu_tensor(tensor: torch.Tensor) -> bool:
...
def _split_tensor_dict(
    ...
    """Split tensors and metadata from a possibly nested dictionary."""
    ...
    for key, value in tensor_dict.items():
        if "%" in key:
            raise ValueError("'%' is reserved as the nested key separator")
        ...
        if isinstance(value, torch.Tensor):
            metadata_list.append(
                (
                    metadata_key,
                    TensorMetadata(value.device.type, value.dtype, value.size()),
                )
            )
        ...
        if isinstance(value, dict):
            ...
            inner_metadata, inner_tensors = _split_tensor_dict(
                value, f"{metadata_key}%"
            )
            metadata_list.extend(inner_metadata)
            tensor_list.extend(inner_tensors)
            continue

        metadata_list.append((metadata_key, value))
    return metadata_list, tensor_list


def _update_nested_dict(
    nested_dict: TensorDict, flattened_key: str, value: Any
) -> None:
    keys = flattened_key.split("%")
    current: TensorDict = nested_dict
    for key in keys[:-1]:
...
class GroupCoordinator:
    ...
    def __init__(
        ...
        self.device_group: Optional[ProcessGroup] = None
        self.cpu_group: Optional[ProcessGroup] = None
        self.device_groups: List[ProcessGroup] = []
        self.cpu_groups: List[ProcessGroup] = []
        self.shm_broadcaster = None

        for ranks in group_ranks:
            device_group = (
                comm_pools[tuple(sorted(ranks))]
                if is_serving and comm_pools is not None
        ...
        if torch.cuda.is_available():
            self.device = torch.device("cuda:0" if is_serving else f"cuda:{local_rank}")
        else:
...
class PipelineGroupCoordinator(GroupCoordinator):
    ...
    def __init__(
        ...
        if len(group_ranks[0]) > 2 or len(group_ranks[0]) == 1:
            for ranks in group_ranks:
                if is_serving:
                    ...
                    device_group = comm_pools[ranks_key]
                else:
        ...
        #   device 0.
            ...
            for ranks in group_ranks:
                if is_serving:
                    ranks_key = tuple(sorted(ranks))
                    device_group_0_1 = comm_pools[ranks_key]
                else:
        ...
        if torch.cuda.is_available() and is_serving:
    ...
    def reset_buffer(self):
        ...
        self.send_shape = {}
        self.recv_buffer = {}
        self.extra_tensors_recv_buffer = {}
        ...
        if recv_flag:
            ...
            if self.recv_buffer.get(name, None) is None:
                ...
                if self.recv_buffer[name].get(segment_idx, None) is not None:
                    logger.warning(
                        "Recv buffer for name '%s' segment %d already exists. Updating buffer.",
                        name,
                        segment_idx,
                    )
    ...
    def recv_skip_next(self):
        if not self.recv_skip_tasks_queue:
            raise ValueError("No more tasks to receive")

        idx = self.recv_skip_tasks_queue.pop(0)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if HAS_LONG_CTX_ATTN:
            ...
            if ulysses_group is not None and ring_group is not None:
