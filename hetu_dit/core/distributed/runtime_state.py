...
from hetu_dit.utils import get_gpu_metadata
...
class RuntimeState(metaclass=ABCMeta):
    ...
    ready: bool = False
    p2p_state: str = "free"  # "free" or "adjusting" or "computing"

    def __init__(self, config: EngineConfig, is_serving: bool = False):
    ...
    def set_p2p_state(self, state: str):
        self.p2p_state = state

    def get_p2p_state(self):
        return self.p2p_state
...
class DiTRuntimeState(RuntimeState):
    ...
    def __init__(
        ...
        cur_metadata=None,
        text_encoder_cur_metadata=None,
    ):  # TODO: text encoder parallel: add cur_text_encoder_metadata
        """
        Initialize DiTRuntimeState.
        Args:
            pipeline: DiffusionPipeline instance.
            config: EngineConfig instance.
            is_serving: Whether in serving mode.
            cur_metadata: Metadata for current GPU.
            text_encoder_cur_metadata: Metadata for text encoder parallel.
        """
        super().__init__(config, is_serving)
        ...
        if pipeline.__class__.__name__.startswith(
            ("CogVideoX", "ConsisID", "HunyuanVideo")
        ):
            ...
            if (
                pipeline.__class__.__name__.startswith("Flux")
                and diffusers.__version__ >= "0.32"
            ):
                vae_scale_factor *= 2
        ...
        self.text_encoder_cur_metadata = text_encoder_cur_metadata
    ...
    def set_input_parameters(
        ...
        self.split_text_embed_in_sp = False  # TODO: need to change to self.split_text_embed_in_sp = split_text_embed_in_sp
        ...
        if (
            ...
            logger.debug("must come here when do new instance")
    ...
    def set_video_input_parameters(
        ...
        logger.info(f"num_inference_steps {self.input_config.num_inference_steps}")
        if self.runtime_config.warmup_steps > self.input_config.num_inference_steps:
            logger.info(
                "self.runtime_config.warmup_steps > self.input_config.num_inference_steps"
            )
            self.runtime_config.warmup_steps = self.input_config.num_inference_steps
            logger.info(f"warmup_steps {self.runtime_config.warmup_steps}")
        self.split_text_embed_in_sp = False  # TODO: need to change to self.split_text_embed_in_sp = split_text_embed_in_sp
        if seed is not None and seed != self.input_config.seed:
            self.input_config.seed = seed
            set_random_seed(seed)
    ...
    def _input_size_change(
        ...
        """
        Handle input size change and recalculate patch metadata.
        Args:
    ...
    def _calc_patches_metadata(self):
        ...
        logger.debug(
            f"in calc_patches_metadata, num_sp_patches = {num_sp_patches}, sp_patch_idx = {sp_patch_idx}"
        )
        ...
        self.num_pipeline_patch = self.parallel_config.pp_config.num_pipeline_patch
        logger.debug(
            f"in calc_patches_metadata, num_pipeline_patch = {self.num_pipeline_patch}"
        )
        # Pipeline patches
        ...
        logger.debug(
            f"in calc_patches_metadata, self.pp_patches_token_num = {self.pp_patches_token_num}, self.num_pipeline_patch = {self.num_pipeline_patch}, self.pp_patches_height = {self.pp_patches_height}, self.pp_patches_start_idx_local = {self.pp_patches_start_idx_local}, self.pp_patches_start_end_idx_global = {self.pp_patches_start_end_idx_global}, self.pp_patches_token_start_idx_local = {self.pp_patches_token_start_idx_local}, self.pp_patches_token_start_end_idx_global = {self.pp_patches_token_start_end_idx_global} "
        )
    ...
    def _calc_cogvideox_patches_metadata(self):
        ...
        sp_patch_idx = get_sequence_parallel_rank()
        logger.info(
            f"in calc_cogvideox_patches_metadata, num_sp_patches = {num_sp_patches}, sp_patch_idx = {sp_patch_idx}"
        )
        patch_size = self.backbone_patch_size
    ...
    def _set_metadata(self, new_metadata):
        self.cur_metadata = new_metadata

    def _set_text_encoder_metadata(self, new_metadata):
        self.text_encoder_cur_metadata = new_metadata

    def _reset_DiTRuntimeState(self, config: EngineConfig):
        self.parallel_config = config.parallel_config
        self.runtime_config = config.runtime_config
        self.input_config = InputConfig()
        self.num_pipeline_patch = self.parallel_config.pp_config.num_pipeline_patch
...
def initialize_runtime_state(
    pipeline: DiffusionPipeline, engine_config: EngineConfig, is_serving: bool = False
):
    global _RUNTIME
    ...
    if hasattr(pipeline, "transformer"):
        logger.debug(
            f"pipeline_parallel_rank is {get_pipeline_parallel_rank()}, pipeline_parallel_world_size is {get_pipeline_parallel_world_size()}, tensor_model_parallel_rank is {get_tensor_model_parallel_rank()}, tensor_model_parallel_world_size is {get_tensor_model_parallel_world_size()}"
        )
        meta_data = get_gpu_metadata(
            get_pipeline_parallel_rank(),
            get_pipeline_parallel_world_size(),
            get_tensor_model_parallel_rank(),
            get_tensor_model_parallel_world_size(),
        )
        logger.debug(f"meta_data is {meta_data}")
    else:
        meta_data = None

    if engine_config.runtime_config.use_parallel_text_encoder and (
        hasattr(pipeline, "text_encoder_3")
        or (
            not hasattr(pipeline, "text_encoder_3")
            and hasattr(pipeline, "text_encoder")
        )
        or (
            not hasattr(pipeline, "text_encoder_3")
            and not hasattr(pipeline, "text_encoder")
            and hasattr(pipeline, "text_encoder_2")
        )
    ):
        logger.debug(
            f"text_encoder_parallel tensor_model_parallel_rank is {get_text_encoder_tensor_model_parallel_rank()}, tensor_model_parallel_world_size is {get_text_encoder_tensor_model_parallel_world_size()}"
        )
        text_encoder_meta_data = get_gpu_metadata(
            0,
            1,
            get_text_encoder_tensor_model_parallel_rank(),
            get_text_encoder_tensor_model_parallel_world_size(),
        )
        logger.debug(f"text_encoder_meta_data is {meta_data}")
    else:
        text_encoder_meta_data = None
    _RUNTIME = DiTRuntimeState(
        pipeline=pipeline,
        config=engine_config,
        is_serving=is_serving,
        cur_metadata=meta_data,
        text_encoder_cur_metadata=text_encoder_meta_data,
    )


def reset_runtime_state(config: EngineConfig):
    global _RUNTIME
    if _RUNTIME is not None:
        _RUNTIME._reset_DiTRuntimeState(config=config)
        logger.debug("Runtime state has been reset.")
    else:
        logger.warning("Runtime state has not been initialized yet.")
