...
from hetu_dit.core.distributed.runtime_state import runtime_state_is_initialized
...
class CacheEntry:
    def __init__(
        ...
        if tensors is None:
            ...
            assert num_cache_tensors == len(tensors), (
...
class CacheManager:
    ...
    def update_and_get_kv_cache(
        ...
        **kwargs,
        ...
        """
        Update the key-value cache for a layer and retrieve the updated cache.

        Args:
            new_kv (Union[torch.Tensor, List[torch.Tensor]]): New key-value tensor(s) to update the cache.
            layer: The layer object.
            slice_dim (int): Dimension along which to slice the cache.
            layer_type (str): Type of the layer.
            custom_get_kv (Callable, optional): Custom function to get kv cache.
            **kwargs: Additional arguments for custom_get_kv.

        Returns:
            torch.Tensor or List[torch.Tensor]: The updated key-value cache.
        """
        return_list = False
    ...
    def _reset_cache(
        ...
        """
        Notes: added for serving and switch model parallel env
        """
        self.cache: Dict[Tuple[str, Any], CacheEntry] = {}
